...
static void update_player(struct world *mzx_world, int player_index)
{
  ...
  switch(under_id)
  {
    case ICE:
    {
      int player_last_dir = src_board->player_last_dir[player_index];
      if(player_last_dir & 0x0F)
      {
        if(move_player(mzx_world, player_index, (player_last_dir & 0x0F) - 1))
          src_board->player_last_dir[player_index] = player_last_dir & 0xF0;
      }
      break;
    }
...
static void give_potion(struct world *mzx_world, enum potion type)
{
  ...
  switch(type)
  {
    ...
    case POTION_INVINCO:
    {
      ...
      send_robot_def(mzx_world, 0, LABEL_INVINCO, -1);
...
#define FPS_INTERVAL 1000
    ...
    for(player_index = 0; player_index < mzx_world->player_count; player_index++)
    {
        ...
        if((!mzx_world->player[player_index].reload) && (!src_board->player_attack_locked))
        {
          ...
          if(move_dir != -1)
          {
            ...
            else
            {
              ...
              src_board->player_last_dir[player_index] =
               (src_board->player_last_dir[player_index] & 0x0F) | (move_dir << 4);
            }
          }
        }
      }
       ...
       (!src_board->player_ns_locked))
      {
        ...
        if((key_down_delay == 0) || (key_down_delay > REPEAT_WAIT))
        {
          ...
          src_board->player_last_dir[player_index] =
           (src_board->player_last_dir[player_index] & 0x0F) + 0x10;
        }
       ...
       (!src_board->player_ew_locked))
      {
        ...
        if((key_right_delay == 0) || (key_right_delay > REPEAT_WAIT))
        {
          ...
          src_board->player_last_dir[player_index] =
           (src_board->player_last_dir[player_index] & 0x0F) + 0x20;
        }
       ...
       (!src_board->player_ew_locked))
      {
        ...
        if((key_left_delay == 0) || (key_left_delay > REPEAT_WAIT))
        {
          ...
          src_board->player_last_dir[player_index] =
           (src_board->player_last_dir[player_index] & 0x0F) + 0x30;
        }
  ...
  if(mzx_world->target_where != TARGET_NONE)
  {
    ...
    else
    {
      ...
      find_player(mzx_world);
      place_player_xy(mzx_world, -1, target_x, target_y);
    }
      ...
      if(dir_mask || (d_flag & A_SPEC_PUSH))
      {
        ...
        if(d_id == ROBOT_PUSHABLE)
          send_robot_def(mzx_world,
           src_board->level_param[d_offset], LABEL_TOUCH, player_index);
...
int grab_item(struct world *mzx_world, int player_index, int offset, int dir)
{
  ...
  switch(id)
  {
    ...
    case GOOP:
    {
      ...
      send_robot_def(mzx_world, 0, LABEL_GOOPTOUCHED, player_index);
      break;
    }
    ...
    case ENERGIZER:
    {
      ...
      send_robot_def(mzx_world, 0, LABEL_INVINCO, player_index);
      set_counter(mzx_world, "INVINCO", 113, 0);
      remove = 1;
      break;
    }
    ...
    case ROBOT:
    {
      ...
      send_robot_def(mzx_world, param, LABEL_TOUCH, player_index);
      break;
    }
